import {signUp,signIn,signOut,sessionCookie,clearedCookie,requestPasswordReset,resetPassword,limit,clientKey} from "@/lib/auth";
...
export async function POST(req:Request){
 try{
  ...
  if(c.action==="signup"){await limit("signup",clientKey(req));const {user,token}=await signUp(c);return json({ok:true,user:{id:user.userId,name:user.fullName}},200,sessionCookie(token))}
