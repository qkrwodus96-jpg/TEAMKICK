...
export function AppDialogs(p:any){
 ...
 {manager&&g.status!=="cancelled"&&<div className="action-strip"><button className="btn" onClick={()=>setModal({kind:"sideSettings",gameId:g.id})}>팀 안내 · 투표 마감 수정</button>{captain&&!g.away&&!g.external&&g.listing!=="open"&&<button className="btn" onClick={()=>setModal({kind:"setOpponent",gameId:g.id})}>외부 상대팀 입력</button>}{captain&&g.status==="scheduled"&&Date.now()<Date.parse(g.start)&&<button className="btn" onClick={()=>setModal({kind:"changeGame",gameId:g.id})}>일정 변경{g.away?" 제안":""}</button>}{captain&&g.status==="scheduled"&&Date.now()>=Date.parse(g.end)&&<button className="btn btn-green" disabled={busy} onClick={()=>run({type:"completeGame",teamId:v.teamId,gameId:g.id})}>경기 완료 처리</button>}{captain&&!g.away&&!g.external&&g.status==="scheduled"&&<button className="btn" disabled={busy} onClick={()=>run({type:g.listing==="open"?"closeListing":"openListing",teamId:v.teamId,gameId:g.id})}>{g.listing==="open"?"상대팀 모집 마감":"상대팀 모집"}</button>}{g.status==="scheduled"&&Date.now()<Date.parse(g.start)&&<button className="btn" disabled={busy} onClick={()=>guestStatusOf(side)==="open"?run({type:"closeGuests",teamId:v.teamId,gameId:g.id}):setModal({kind:"openGuests",gameId:g.id})}>{guestStatusOf(side)==="open"?"용병 모집 마감":"용병 모집"}</button>}{captain&&<button className="btn btn-danger" onClick={()=>setModal({kind:"reason",command:"cancelGame",teamId:v.teamId,gameId:g.id,title:"경기 취소"})}>경기 취소</button>}</div>}
 ...
 {modal?.kind==="matchRecord"&&(()=>{
  ...
  return <form className="form-grid" onSubmit={e=>{e.preventDefault();save({type:"matchRecord",teamId:v.teamId,gameId:modal.gameId,values:form.values,ownGoals:own,unknownGoals:unknown,opponent:against})}}>
   ...
   <TableBody>{rows.map(([mid,rec])=><TableRow key={mid}><TableCell>{roster.find((m:Row)=>m.id===mid)?.name??"선수"}</TableCell>
    {(["goals","assists"] as const).map(k=><TableCell key={k}><input aria-label={mid+" "+k} type="number" min="0" max="99" className="record-input" value={rec[k]} onChange={e=>field("values",{...form.values,[mid]:{...rec,[k]:e.target.value===""?0:Number(e.target.value)}})}/></TableCell>)}
