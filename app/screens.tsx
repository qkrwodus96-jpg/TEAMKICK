...
export function AppDialogs(p:any){
 ...
 {(modal?.kind==="createTeam"||modal?.kind==="editTeam")&&<form className="form-grid" onSubmit={e=>{e.preventDefault();save({...form,type:modal.kind,teamId:v.teamId})}}>{label("팀 이름","name")}<label>활동 지역<Picker value={form.region} onChange={(x:string)=>field("region",x)} options={REGIONS}/></label>{modal.kind==="createTeam"&&<><label>주 경기 형식<Picker value={form.format} onChange={(x:string)=>field("format",x)} options={["11인제","8인제","6인제","5인제"]}/></label>{label("활동 요일","days")}</>}<label>팀 소개<textarea value={form.description??""} onChange={e=>field("description",e.target.value)}/></label>{submit(modal.kind==="createTeam"?"운영자에게 등록 신청":"팀 정보 저장")}</form>}
