...
import {Upload,Plus,Search,MapPin,CalendarDays,Clock,Users,ShieldCheck,Copy,ExternalLink,Settings,Check,CheckCircle2,X,ArrowLeft,LogOut,Download,LoaderCircle,Goal,Handshake,Bell} from "lucide-react";
import {Picker,Crest,imageUrl,Empty,GameBadge,GuestBadge,PlayerPhoto,Vote,koreanDate,time,localDay,inputTime,fromInput,opponent,scoreText,started,backNo,backNoOr,NoticeList} from "./teamkick";
...
export function AuthPanel({onDemo,mailReady=true,kakaoReady=false,googleReady=false,naverReady=false,emailSignupEnabled=false,resetToken="",onResetCancel,inviteTeam=""}:{onDemo:()=>void;onResetCancel?:()=>void;inviteTeam?:string;emailSignupEnabled?:boolean;mailReady?:boolean;kakaoReady?:boolean;googleReady?:boolean;naverReady?:boolean;resetToken?:string}){
 ...
 return <section className="onboarding panel">{inviteTeam&&!resetToken&&<div className="task-box invite-note" role="status"><strong>{inviteTeam} 팀에서 초대했어요</strong><p className="small muted">로그인하거나 가입하면 바로 <b>{inviteTeam}</b> 가입 신청 창이 떠요. 주장이 승인하면 팀 공간이 열려요.</p></div>}
...
async function closeWithProvider(provider:string){
 try{
  const res=await fetch("/api/"+provider,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({action:"close"})});
  const out=await res.json().catch(()=>({})) as {location?:string;error?:string};
...
export function BackupPanel(){
 ...
 async function restore(e:FormEvent){
  ...
  try{
   ...
   const out=await res.json().catch(()=>({})) as {error?:string;accounts?:number;forgotten?:number;exportedAt?:string};
   if(!res.ok)throw new Error(out.error||"복원하지 못했어요.");
   toast.success("복원했어요. 계정 "+(out.accounts??0)+"개를 되살렸어요."+(out.forgotten?" 백업 뒤에 탈퇴한 "+out.forgotten+"명은 되살리지 않았어요.":"")+" 화면을 새로고침해주세요.");
...
export function Matching(p:any){
 ...
 const guestListings=(v.guestListings??[]).filter((z:Row)=>
   (region==="all"||z.region===region)&&(format==="all"||z.format===format)
...
export function Management(p:any){
 ...
 {captain&&pending.length>0&&<section className="panel"><h2 className="view-heading">가입 승인 대기 <span className="badge badge-orange">{pending.length}</span></h2>{pending.map((m:Row)=><div key={m.id} className="attendance-item"><div><strong>{m.name}</strong><p className="data-note">{m.position} · 희망 {backNoOr(m.number)}</p></div><div className="row"><button className="btn btn-green" disabled={busy} onClick={()=>run({type:"approveMember",teamId:v.teamId,memberId:m.id})}>승인</button><button className="btn" disabled={busy} onClick={()=>run({type:"rejectMember",teamId:v.teamId,memberId:m.id})}>거절</button></div></div>)}</section>}
 ...
 <section className="panel"><div className="panel-title"><h2>팀 공지</h2>{captain&&<button className="btn" onClick={()=>setModal({kind:"notice"})}><Plus/>공지 작성</button>}</div><NoticeList notices={v.notices??[]} onOpen={(n:Row)=>setModal({kind:"noticeDetail",notice:n})}/></section><MyHub {...p}/></div>
}
...
export function AppDialogs(p:any){
 const {modal,setModal,v,team,demo,busy,action,run,manager,captain,setView,setJumpTab}=p,[form,setForm]=useState<any>({}),[detailTab,setDetailTab]=useState("info"),[failure,setFailure]=useState(""),[confirm,setConfirm]=useState<any>(null),[install,setInstall]=useState<any>(null);
 ...
 useEffect(()=>{setFailure("");if(!modal)return;setDetailTab(modal.tab??"info");const game=v.games?.find((g:Row)=>g.id===modal.gameId),z=v.sides?.find((s:Row)=>s.gameId===modal.gameId);
  ...
  if(modal.kind==="matchRecord"&&isIntra(game)){
   ...
   f={values,extra:Array.from({length:game.squads||2},(_,i)=>z.squadExtra?.[i]??0)};
  }
  else if(modal.kind==="matchRecord"){
   // 우리 팀 점수는 아래에서 더해서 구한다. 손으로 넣는 것은 상대팀 득점뿐이다.
   const values=Object.fromEntries((z.roster??[]).filter((m:Row)=>z.attendance[m.id]).map((m:Row)=>[m.id,z.records[m.id]??{goals:0,assists:0}]));
   const against=game.home===v.teamId?game.result?.b:game.result?.a;
   f={values,ownGoals:z.ownGoals??0,unknownGoals:z.unknownGoals??0,opponent:typeof against==="number"?against:0};
  }
  ...
  if(modal.kind==="profile"||modal.kind==="editMember")f={...modal.member};if(modal.kind==="joinTeam")f={name:v.user?.name==="팀원"?"":v.user?.name,position:"MF",number:0,readRules:false};if(modal.kind==="rules")f={rules:team?.rules??"",notify:true};
 ...
 const TAB_LABEL:Record<string,string>={guest:"용병",received:"받은 신청",mine:"우리 팀 매칭 내역",open:"모집 중",confirmed:"확정 매칭"};
 const targetOf=(n:Row)=>{
  const [view,tab]=String(n.to||(n.gameId?"schedule":"home")).split(":");
  return {view,tab:tab||""};
 };
 const viewOf=(n:Row)=>targetOf(n).view;
 const viewLabel=(n:Row)=>{const {view,tab}=targetOf(n);return (VIEW_LABEL[view]??"홈")+(tab&&TAB_LABEL[tab]?" · "+TAB_LABEL[tab]:"")};
 function openNotification(n:Row){
  const {view,tab}=targetOf(n);
  setView(view);
  // 매칭처럼 탭이 여럿인 화면은 어느 탭인지까지 알려 준다.
  if(tab)setJumpTab(view+":"+tab);
  // 서버가 적어 준 화면이 먼저다. 매칭 신청처럼 경기가 딸려 있어도 갈 곳은 매칭이다.
  // 일정으로 가는 알림일 때만 그 경기 창까지 바로 열어 준다.
  if(view==="schedule"&&n.gameId&&v.games.some((g:Row)=>g.id===n.gameId))setModal({kind:"game",id:n.gameId});
  else setModal(null);
 }
 ...
 return <><Dialog open={!!modal} onOpenChange={o=>!o&&closed()}><DialogContent className={modal?.kind==="game"||modal?.kind==="records"||modal?.kind==="matchRecord"?"sm:max-w-[720px] max-h-[90vh] overflow-y-auto rounded-2xl":"sm:max-w-[520px] max-h-[90vh] overflow-y-auto rounded-2xl"}><DialogHeader><DialogTitle>{modal?.kind==="game"?"경기 상세":titles[modal?.kind]??"팀킥"}</DialogTitle><DialogDescription>{modal?.kind==="game"&&g?koreanDate(g.start)+" · "+time(g.start):modal?.kind==="setup"?"전달받은 초기 설정 코드로 서비스 운영자를 등록합니다.":demo?"샘플 팀 공간입니다. 실제 데이터에는 반영되지 않습니다.":subtitles[modal?.kind]??"우리 팀의 정보를 확인하고 관리하세요."}</DialogDescription></DialogHeader>
 {failure&&<p className="error-bar" role="alert">{failure}</p>}
 {modal?.kind==="game"&&g&&<><div className="detail-score"><div className="club"><Crest name={team?.name} logo={team?.logo}/><strong>{team?.name}</strong></div><div style={{textAlign:"center"}}><GameBadge g={g}/><div className={"score-big"+(scoreText(v,g)?"":" score-word")} style={{marginTop:12}}>{scoreText(v,g)||(isIntra(g)?"자체전":"VS")}</div>{isIntra(g)&&g.result?.status==="confirmed"&&<p className="small muted" style={{marginTop:4}}>{SQUAD_NAMES.slice(0,g.squads).join(" : ")}</p>}</div>{isIntra(g)?<div className="club"><div className="squad-crest" aria-hidden>{SQUAD_NAMES.slice(0,g.squads).map((x:string)=><i key={x}>{x.slice(0,1)}</i>)}</div><strong>자체전</strong></div>:<div className="club"><Crest name={opponent(v,g)} color="orange" logo={v.teams.find((t:Row)=>t.id===(g.home===v.teamId?g.away:g.home))?.logo}/><strong>{opponent(v,g)||"상대팀 미정"}</strong></div>}</div><Tabs value={detailTab} onValueChange={setDetailTab}><TabsList className="mb-4 detail-tabs"><TabsTrigger value="info">경기 정보</TabsTrigger><TabsTrigger value="attendance">참여 현황</TabsTrigger>{isIntra(g)&&<TabsTrigger value="squads">팀 나누기</TabsTrigger>}<TabsTrigger value="record">경기 기록</TabsTrigger>{g.status==="completed"&&<TabsTrigger value="mvp">MVP</TabsTrigger>}</TabsList>
 <TabsContent value="info"><div className="detail-meta"><span className="meta-pair"><CalendarDays/>{koreanDate(g.start)} · {time(g.start)} – {time(g.end)}</span><span className="meta-pair"><MapPin/>{g.venue}</span><p className="small">{g.address}</p><div className="row"><button className="btn" onClick={()=>copy(g.address)}><Copy/>주소 복사</button><a className="btn" target="_blank" rel="noopener noreferrer" href={"https://map.naver.com/p/search/"+encodeURIComponent(g.address)}><ExternalLink/>지도 보기</a></div><p>{g.format} · {g.secured?"구장 확보 완료":"구장 협의 중"} · 비용 안내 {Number(g.cost).toLocaleString()}원</p><p className="meta-pair"><Handshake/>{isIntra(g)?"자체전 · "+SQUAD_NAMES.slice(0,g.squads).join("·")+"으로 나눠 뛰어요":g.away?"상대팀 모집 마감 · 매칭 확정 ("+(opponent(v,g)||"상대팀")+")":g.external?"상대팀 모집 안 함 · 외부 상대팀 "+g.external:g.listing==="open"?"상대팀 모집 중":g.listing==="closed"?"상대팀 모집 마감 · 아직 상대팀이 없어요":"상대팀 미정 · 아직 모집하지 않았어요"}</p>{g.external&&<p>팀 자체 기록으로 결과를 남겨요.</p>}{side?.deadline&&<p className="meta-pair"><Clock/>투표 마감 {koreanDate(side.deadline)} {time(side.deadline)}{Date.now()>=Date.parse(side.deadline)?" · 마감됨":""}{manager&&g.status==="scheduled"&&!started(g)&&<button type="button" className="text-link" style={{marginLeft:8}} onClick={()=>setModal({kind:"sideSettings",gameId:g.id})}>수정</button>}</p>}{side?.meeting&&<p>집합 시간 {side.meeting}</p>}{side?.note&&<p style={{whiteSpace:"pre-wrap"}}>{side.note}</p>}</div>
 ...
 {manager&&g.status!=="cancelled"&&<div className="action-strip"><button className="btn" onClick={()=>setModal({kind:"sideSettings",gameId:g.id})}>팀 안내 · 투표 마감 수정</button>{captain&&!isIntra(g)&&!g.away&&!g.external&&g.listing!=="open"&&<button className="btn" onClick={()=>setModal({kind:"setOpponent",gameId:g.id})}>외부 상대팀 입력</button>}{captain&&g.status==="scheduled"&&Date.now()<Date.parse(g.start)&&<button className="btn" onClick={()=>setModal({kind:"changeGame",gameId:g.id})}>일정 변경{g.away?" 제안":""}</button>}{captain&&g.status==="scheduled"&&Date.now()>=Date.parse(g.end)&&<button className="btn btn-green" disabled={busy} onClick={()=>run({type:"completeGame",teamId:v.teamId,gameId:g.id})}>경기 완료 처리</button>}{captain&&!isIntra(g)&&!g.away&&!g.external&&g.status==="scheduled"&&<button className="btn" disabled={busy} onClick={()=>run({type:g.listing==="open"?"closeListing":"openListing",teamId:v.teamId,gameId:g.id})}>{g.listing==="open"?"상대팀 모집 마감":"상대팀 모집"}</button>}{g.status==="scheduled"&&Date.now()<Date.parse(g.start)&&<button className="btn" disabled={busy} onClick={()=>guestStatusOf(side)==="open"?run({type:"closeGuests",teamId:v.teamId,gameId:g.id}):setModal({kind:"openGuests",gameId:g.id})}>{guestStatusOf(side)==="open"?"용병 모집 마감":"용병 모집"}</button>}{captain&&<button className="btn btn-danger" onClick={()=>setModal({kind:"reason",command:"cancelGame",teamId:v.teamId,gameId:g.id,title:"경기 취소"})}>경기 취소</button>}</div>}
 {guestStatusOf(side)!=="none"&&<div className="panel" style={{marginTop:15}}><div className="row between"><strong>용병 모집</strong><GuestBadge z={side} g={g}/></div><p className="data-note">승인 {(v.guests??[]).filter((x:Row)=>x.gameId===g.id&&x.status==="approved").length}명 / 모집 {side?.guestNeeded}명 · 승인 대기자는 인원에 포함되지 않아요.</p>{(v.guests??[]).filter((x:Row)=>x.gameId===g.id&&["pending","approved"].includes(x.status)).map((x:Row)=><div className="attendance-item" key={x.id}><div><strong>{x.name}</strong><p className="data-note">{x.position} · {backNoOr(x.number)}{x.message?" · "+x.message:""}</p></div><div className="row">{x.status==="approved"&&<span className="badge badge-green">확정</span>}{manager&&(x.status==="pending"?<><button className="btn btn-green" disabled={busy} onClick={()=>run({type:"approveGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>승인</button><button className="btn" disabled={busy} onClick={()=>run({type:"rejectGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>거절</button></>:<button className="btn" disabled={busy} onClick={()=>run({type:"cancelGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>취소</button>)}</div></div>)}{!(v.guests??[]).some((x:Row)=>x.gameId===g.id&&["pending","approved"].includes(x.status))&&<p className="small muted">아직 신청한 용병이 없어요.</p>}</div>}
 ...
 {(side?.guestRoster??[]).map((m:Row)=><div className="attendance-item" key={m.id}><span>{m.name} <small className="muted">{backNo(m.number)?backNo(m.number)+" · ":""}{m.position}</small></span><span className="badge badge-orange">용병 참여</span></div>)}<p className="data-note">투표는 참석 의사입니다. 경기 완료 후 실제 참석 여부를 확인하면 출석에 반영됩니다.</p>{manager&&g.status==="scheduled"&&<div className="action-strip"><button className="btn" disabled={busy||!(side?.roster??[]).some((m:Row)=>currentVote(side,m.id)==="none")} onClick={()=>save({type:"remindVote",teamId:v.teamId,gameId:g.id},(out:{notified?:number})=>{toast.success(out?.notified+"명에게 알림을 보냈어요.");setModal(null)})}>미응답자에게 알림 보내기</button></div>}{manager&&g.status==="scheduled"&&<p className="data-note">앱 알림함으로 전달돼요. 같은 경기는 6시간에 한 번 보낼 수 있어요.</p>}{manager&&g.status==="completed"&&<button className="btn btn-green" style={{marginTop:18}} onClick={()=>setModal({kind:"attendance",gameId:g.id})}>{side.attendanceFinal?"출석 수정":"출석 확인 · 확정"}</button>}</TabsContent>
 {isIntra(g)&&<TabsContent value="squads"><SquadBoard key={g.id+":"+JSON.stringify(side?.squads??{})} g={g} side={side} v={v} canEdit={manager&&g.status!=="cancelled"&&team?.status==="active"} busy={busy} vote={currentVote} onSave={(assign:Record<string,number>)=>save({type:"squads",teamId:v.teamId,gameId:g.id,assign},()=>toast.success("팀을 나눴어요."))}/></TabsContent>}
 {g.status==="completed"&&<TabsContent value="mvp"><MvpPanel key={g.id+":"+(side?.mvp?.mine??"")} g={g} side={side} v={v} manager={manager} busy={busy} save={save}/></TabsContent>}
 {isIntra(g)?<TabsContent value="record"><p className="status-line">{g.result?.status==="confirmed"?"자체전 기록 확정":"자체전 기록 전"}</p>{g.result?.status==="confirmed"&&<div className="squad-score">{SQUAD_NAMES.slice(0,g.squads).map((n:string,i:number)=><div key={n} className={"squad-score-item squad-"+i}><span>{n}</span><b>{g.result.squads?.[i]??0}</b></div>)}</div>}
  <Table className="roster-table"><TableHeader><TableRow><TableHead>선수</TableHead><TableHead>팀</TableHead><TableHead>골</TableHead><TableHead>도움</TableHead></TableRow></TableHeader><TableBody>{(Object.entries(side?.records??{}) as [string,{goals:number;assists:number}][]).filter(([,r])=>r.goals||r.assists).map(([mid,rec])=><TableRow key={mid}><TableCell>{side.roster?.find((m:Row)=>m.id===mid)?.name??"과거 선수"}</TableCell><TableCell>{typeof side.squads?.[mid]==="number"?SQUAD_NAMES[side.squads[mid]]:"—"}</TableCell><TableCell>{rec.goals}</TableCell><TableCell>{rec.assists}</TableCell></TableRow>)}</TableBody></Table>
  <div className="action-strip">{manager&&g.status==="completed"&&side?.attendanceFinal&&<button className="btn btn-green" onClick={()=>setModal({kind:"matchRecord",gameId:g.id})}>{side?.recordsFinal?"자체전 기록 고치기":"자체전 기록 입력"}</button>}<button className="btn" onClick={()=>setModal({kind:"correct",gameId:g.id})}>기록 정정 요청</button></div>
  {manager&&g.status==="completed"&&!side?.attendanceFinal&&<p className="data-note">참여 현황에서 <strong>출석을 먼저 확정</strong>하면 골·도움을 넣을 수 있어요. MVP 투표도 그때 열려요.</p>}
  <p className="data-note">골을 넣은 선수는 <strong>팀 나누기</strong>에서 몇 팀인지 정해져 있어야 해요. 팀별 점수는 선수 골을 더해서 구해요.</p></TabsContent>:
 ...
 <Table className="roster-table"><TableHeader><TableRow><TableHead>선수</TableHead><TableHead>골</TableHead><TableHead>어시스트</TableHead></TableRow></TableHeader><TableBody>{Object.entries(side?.records??{}).map(([mid,rec]:any)=><TableRow key={mid}><TableCell>{side.roster?.find((m:Row)=>m.id===mid)?.name??v.members.find((m:Row)=>m.id===mid)?.name??"과거 선수"}</TableCell><TableCell>{rec.goals}</TableCell><TableCell>{rec.assists}</TableCell></TableRow>)}</TableBody></Table><p className="data-note">{side?.recordsFinal?"개인 기록 확정":"개인 기록 미확정"} · 상대 자책골 {side?.ownGoals??0} · 득점자 미상 {side?.unknownGoals??0}</p><div className="action-strip">{captain&&g.status==="completed"&&side?.attendanceFinal&&(g.away||g.external)&&<button className="btn btn-green" onClick={()=>setModal({kind:"matchRecord",gameId:g.id})}>경기 기록 입력</button>}{captain&&g.status==="completed"&&<button className="btn" onClick={()=>setModal({kind:"result",gameId:g.id})}>점수만 입력</button>}{!captain&&manager&&g.status==="completed"&&side?.attendanceFinal&&g.result?.status==="confirmed"&&<button className="btn btn-green" onClick={()=>setModal({kind:"records",gameId:g.id})}>골 · 도움 입력</button>}<button className="btn" onClick={()=>setModal({kind:"correct",gameId:g.id})}>기록 정정 요청</button></div>{captain&&g.status==="completed"&&!side?.attendanceFinal&&<p className="data-note">참여 현황에서 <strong>출석을 먼저 확정</strong>하면 선수 기록과 점수를 한 번에 넣을 수 있어요.</p>}{captain&&g.status==="completed"&&side?.attendanceFinal&&!g.away&&!g.external&&<p className="data-note">상대팀을 먼저 정해주세요. 경기 정보 탭의 <strong>외부 상대팀 입력</strong>을 쓰면 돼요.</p>}</TabsContent>}</Tabs></>}
 ...
 {(modal?.kind==="joinTeam"||modal?.kind==="profile"||modal?.kind==="editMember")&&<form className="form-grid" onSubmit={e=>{e.preventDefault();save({...form,type:modal.kind==="profile"?"editProfile":modal.kind==="editMember"?"editMember":"joinTeam",teamId:modal.team?.id??v.teamId,memberId:modal.member?.id})}}>{modal.team&&<strong>{modal.team.name}</strong>}{modal.kind==="joinTeam"&&modal.team?.rules&&<div className="rules-preview"><span className="field-label">팀 회칙</span><p className="rules-text">{modal.team.rules}</p><label className="row"><Checkbox checked={!!form.readRules} onCheckedChange={x=>field("readRules",x===true)}/>회칙을 읽었어요</label></div>}{label("선수 이름","name")}<div className="two-col fields">{label("등번호","number","number")}<label>포지션<Picker value={form.position??"MF"} onChange={(x:string)=>field("position",x)} options={["GK","DF","MF","FW"]}/></label></div>{modal.kind==="joinTeam"&&modal.team?.rules&&!form.readRules?<button type="button" className="btn btn-green" disabled>회칙을 읽고 체크해주세요</button>:submit(modal.kind==="joinTeam"?"주장에게 가입 신청":"저장하기")}</form>}
 ...
 {modal?.kind==="matchRecord"&&isIntra(v.games.find((x:Row)=>x.id===modal.gameId))&&(()=>{
  // 자체전: 선수마다 골·도움을 넣으면 팀별 점수가 더해진다. 용병·미상 골은 팀마다 따로.
  const game=v.games.find((x:Row)=>x.id===modal.gameId),z=v.sides.find((x:Row)=>x.gameId===modal.gameId);
  const n=game?.squads||2,rows=Object.entries((form.values??{}) as Record<string,{goals:number;assists:number}>);
  const roster:Row[]=z?.roster??[],squadOf=(id:string)=>z?.squads?.[id];
  const extra:number[]=form.extra??[];
  const totals=Array.from({length:n},(_,i)=>rows.filter(([id])=>squadOf(id)===i).reduce((x,[,r])=>x+Number(r.goals||0),0)+Number(extra[i]||0));
  const setRec=(id:string,k:"goals"|"assists",val:number)=>field("values",{...form.values,[id]:{...form.values[id],[k]:val}});
  const unassigned=rows.filter(([id])=>typeof squadOf(id)!=="number");
  const block=(title:string,list:[string,{goals:number;assists:number}][],i?:number)=><section className={"record-squad"+(i===undefined?" wait":" squad-"+i)} key={title}><header><strong>{title}</strong>{i!==undefined&&<b className="record-squad-score">{totals[i]}</b>}</header>
   {list.map(([id,r])=><div className="record-row" key={id}><span className="record-name">{roster.find(m=>m.id===id)?.name??"선수"}</span><span className="record-steps"><span className="record-k">골</span><Stepper label={"골"} value={r.goals} onChange={x=>setRec(id,"goals",x)} max={i===undefined?0:99}/><span className="record-k">도움</span><Stepper label={"도움"} value={r.assists} onChange={x=>setRec(id,"assists",x)}/></span></div>)}
   {i!==undefined&&<div className="record-row extra"><span className="record-name muted">용병·득점자 미상</span><Stepper label={SQUAD_NAMES[i]+" 용병·미상 골"} value={extra[i]??0} onChange={x=>{const e=[...extra];e[i]=x;field("extra",e)}}/></div>}
   {!list.length&&i!==undefined&&<p className="small muted">이 팀에 나눠진 출석자가 없어요.</p>}</section>;
  return <form className="form-grid" onSubmit={e=>{e.preventDefault();save({type:"matchRecord",teamId:v.teamId,gameId:modal.gameId,values:form.values,extra})}}>
   <div className="squad-score">{SQUAD_NAMES.slice(0,n).map((nm:string,i:number)=><div key={nm} className={"squad-score-item squad-"+i}><span>{nm}</span><b>{totals[i]}</b></div>)}</div>
   {SQUAD_NAMES.slice(0,n).map((nm:string,i:number)=>block(nm,rows.filter(([id])=>squadOf(id)===i),i))}
   {!!unassigned.length&&block("팀 미정 (도움만 기록할 수 있어요)",unassigned)}
   {!!unassigned.length&&<p className="data-note">팀이 정해지지 않은 출석자는 골을 넣을 수 없어요. 경기 상세의 <strong>팀 나누기</strong>에서 먼저 정해주세요.</p>}
   {submit("자체전 기록 저장")}
  </form>;
 })()}
 {modal?.kind==="matchRecord"&&!isIntra(v.games.find((x:Row)=>x.id===modal.gameId))&&(()=>{
  // 우리 팀 점수를 손으로 넣지 않는다. 선수 골 + 상대 자책골 + 득점자 미상을 더한
  // 값이 곧 우리 팀 점수다. 손으로 넣는 숫자는 상대팀 득점 하나뿐이다.
  const rows=Object.entries((form.values??{}) as Record<string,{goals:number;assists:number}>);
  const roster=v.sides.find((x:Row)=>x.gameId===modal.gameId)?.roster??[];
  const scored=rows.reduce((n,[,r])=>n+Number(r.goals||0),0);
  const own=Number(form.ownGoals||0),unknown=Number(form.unknownGoals||0),total=scored+own+unknown;
  const against=Number(form.opponent||0);
  return <form className="form-grid" onSubmit={e=>{e.preventDefault();save({type:"matchRecord",teamId:v.teamId,gameId:modal.gameId,values:form.values,ownGoals:own,unknownGoals:unknown,opponent:against})}}>
   <p className="small muted">출석이 확정된 선수의 골·도움을 먼저 넣으세요. <strong>우리 팀 점수는 아래에서 자동으로 더해집니다.</strong></p>
   <Table className="roster-table"><TableHeader><TableRow><TableHead>선수</TableHead><TableHead>골</TableHead><TableHead>도움</TableHead></TableRow></TableHeader>
   <TableBody>{rows.map(([mid,rec])=><TableRow key={mid}><TableCell>{roster.find((m:Row)=>m.id===mid)?.name??"선수"}</TableCell>
    {(["goals","assists"] as const).map(k=><TableCell key={k}><Stepper label={(roster.find((m:Row)=>m.id===mid)?.name??"선수")+(k==="goals"?" 골":" 도움")} value={rec[k]} onChange={x=>field("values",{...form.values,[mid]:{...rec,[k]:x}})}/></TableCell>)}
   </TableRow>)}</TableBody></Table>
   {!rows.length&&<p className="data-note">출석 확정된 선수가 없어요. 참여 현황에서 출석을 먼저 확정해주세요.</p>}
   <div className="two-col fields">{label("상대 자책골","ownGoals","number")}{label("득점자 미상","unknownGoals","number")}</div>
   <div className="panel score-sum"><div className="row between"><span>우리 팀 득점 <small className="muted">(선수 {scored} + 자책 {own} + 미상 {unknown})</small></span><strong className="score-sum-value">{total}</strong></div></div>
   {label("상대팀 득점","opponent","number")}
   <p className="data-note">최종 {total} : {against} 로 기록됩니다. 앱 안에서 매칭된 경기는 상대 주장이 확인해야 확정되고, 외부 상대팀 경기는 바로 확정돼요.</p>
   {submit("경기 기록 저장")}
  </form>;
 })()}
 ...
 {modal?.kind==="changeGame"&&<form className="form-grid" onSubmit={e=>{e.preventDefault();save({type:"changeGame",teamId:v.teamId,gameId:modal.gameId,...form,start:fromInput(form.start),end:fromInput(form.end)})}}><div className="two-col fields"><label>변경 시작<input type="datetime-local" required value={form.start??""} onChange={e=>startChanged(e.target.value)}/>{whenNote(form.start,"",true)}</label><label>변경 종료<input type="datetime-local" required value={form.end??""} onChange={e=>field("end",e.target.value)}/>{whenNote(form.end,"",true)}</label></div>{whenNote(form.start,form.end)}<VenuePicker v={v} field={field} ready={v.placeSearchReady!==false} demo={demo}/>{label("구장 이름","venue")}{label("구장 주소","address")}{!form.away&&form.listing!=="open"&&label("외부 상대팀 이름","external","text",false)}{label("비용 안내","cost","number")}<p className="data-note">상대팀이 확정되었다면 상대 주장의 동의가 필요합니다. 변경 후 참여 여부를 다시 확인합니다.</p>{submit("일정 변경 제출")}</form>}
 ...
 {modal?.kind==="invite"&&<><p className="small muted">초대받은 사람은 로그인한 뒤 주장의 가입 승인을 받아야 해요. 링크와 QR은 7일 동안 쓸 수 있어요.</p>{form.link?<InviteShare link={form.link} teamName={team?.name??"우리"}/>:<button className="btn btn-green" disabled={busy} onClick={()=>save({type:"invite",teamId:v.teamId},o=>field("link",window.location.origin+"/?invite="+encodeURIComponent(o.invite)))}>초대 QR · 링크 만들기</button>}{v.invites.filter((i:Row)=>i.active).map((i:Row)=><div className="row between" key={i.id}><span className="data-note">{localDay(i.expires)}까지 유효</span><button className="text-link" disabled={busy} onClick={()=>run({type:"revokeInvite",teamId:v.teamId,inviteId:i.id})}>링크 폐기</button></div>)}</>}
 {modal?.kind==="rules"&&<form className="form-grid" onSubmit={e=>{e.preventDefault();save({type:"editRules",teamId:v.teamId,rules:form.rules??"",notify:!!form.notify},()=>{toast.success("회칙을 저장했어요.");setModal(null)})}}>
  <div className="field-block"><span className="field-label">예시 넣기</span><div className="chip-row">{RULE_TEMPLATES.map(t=><button type="button" key={t.label} className="chip-mini" onClick={()=>field("rules",((form.rules??"").trim()?(form.rules??"").trimEnd()+"\n\n":"")+t.text)}><Plus size={13}/>{t.label}</button>)}</div><p className="data-note">누르면 아래에 예시가 붙어요. 우리 팀에 맞게 고쳐 쓰세요.</p></div>
  <label>회칙<textarea className="rules-input" maxLength={RULES_MAX} value={form.rules??""} onChange={e=>field("rules",e.target.value)} placeholder={"[회비]\n- 월 회비 2만 원, 매월 5일까지"}/></label>
