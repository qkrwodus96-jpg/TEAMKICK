...
import {Picker,Crest,imageUrl,Empty,GameBadge,GuestBadge,PlayerPhoto,Vote,koreanDate,time,localDay,inputTime,fromInput,opponent,started} from "./teamkick";
...
export function AppDialogs(p:any){
 ...
 {guestStatusOf(side)!=="none"&&<div className="panel" style={{marginTop:15}}><div className="row between"><strong>용병 모집</strong><GuestBadge z={side} g={g}/></div><p className="data-note">승인 {(v.guests??[]).filter((x:Row)=>x.gameId===g.id&&x.status==="approved").length}명 / 모집 {side?.guestNeeded}명 · 승인 대기자는 인원에 포함되지 않아요.</p>{(v.guests??[]).filter((x:Row)=>x.gameId===g.id&&["pending","approved"].includes(x.status)).map((x:Row)=><div className="attendance-item" key={x.id}><div><strong>{x.name}</strong><p className="data-note">{x.position} · 등번호 {x.number}{x.message?" · "+x.message:""}</p></div><div className="row">{x.status==="approved"&&<span className="badge badge-green">확정</span>}{manager&&(x.status==="pending"?<><button className="btn btn-green" disabled={busy} onClick={()=>run({type:"approveGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>승인</button><button className="btn" disabled={busy} onClick={()=>run({type:"rejectGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>거절</button></>:<button className="btn" disabled={busy} onClick={()=>run({type:"cancelGuest",teamId:v.teamId,gameId:g.id,guestId:x.id})}>취소</button>)}</div></div>)}{!(v.guests??[]).some((x:Row)=>x.gameId===g.id&&["pending","approved"].includes(x.status))&&<p className="small muted">아직 신청한 용병이 없어요.</p>}</div>}
