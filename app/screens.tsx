...
export function Matching(p:any){
 ...
 return <><Tabs value={tab} onValueChange={setTab}><TabsList className="mb-5 flex-wrap h-auto"><TabsTrigger value="open">모집 중</TabsTrigger><TabsTrigger value="mine">우리 팀 매칭 내역</TabsTrigger>{captain&&<TabsTrigger value="received">받은 신청 {received.filter((r:Row)=>r.status==="pending").length||""}</TabsTrigger>}<TabsTrigger value="guest">용병 {teamGuests.filter((x:Row)=>x.status==="pending").length||""}</TabsTrigger><TabsTrigger value="confirmed">확정 매칭</TabsTrigger></TabsList></Tabs>
