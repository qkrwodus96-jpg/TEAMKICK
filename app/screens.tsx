...
import {currentVote,guestStatusOf,REGIONS,FORMATS,LEVELS,levelOf,type Row} from "@/lib/model";
...
export function AppDialogs(p:any){
 ...
 useEffect(()=>{setFailure("");if(!modal)return;setDetailTab("info");const game=v.games?.find((g:Row)=>g.id===modal.gameId),z=v.sides?.find((s:Row)=>s.gameId===modal.gameId);
  let f:any={};if(modal.kind==="createGame"){const d=new Date(Date.now()+7*864e5);f={start:inputTime(d.toISOString()).slice(0,10)+"T10:00",end:inputTime(d.toISOString()).slice(0,10)+"T12:00",format:"11인제",secured:true,needed:11,cost:0,listing:modal.listing??false,external:"",region:team?.region??""};}
 ...
 <TabsContent value="attendance"><div className="row" style={{flexWrap:"wrap",marginBottom:16}}>{[{key:"yes",label:"참여"},{key:"no",label:"미참여"},{key:"maybe",label:"미정"},{key:"none",label:"미응답"}].map(x=><span className="badge" key={x.key}>{x.label} {(side?.roster??[]).filter((m:Row)=>currentVote(side,m.id)===x.key).length}명</span>)}</div>{(side?.roster??[]).map((m:Row)=><div className="attendance-item" key={m.id}><span>{m.name} <small className="muted">#{m.number}</small></span><span className="badge">{side.attendanceFinal?(side.attendance[m.id]?"출석 확정":"불참 확정"):({yes:"참여 예정",no:"미참여",maybe:"미정",none:"미응답"} as any)[currentVote(side,m.id)]}</span></div>)}<p className="data-note">투표는 참석 의사입니다. 경기 완료 후 실제 참석 여부를 확인하면 출석에 반영됩니다.</p>{manager&&g.status==="scheduled"&&<div className="action-strip"><button className="btn" disabled={busy||!(side?.roster??[]).some((m:Row)=>currentVote(side,m.id)==="none")} onClick={()=>save({type:"remindVote",teamId:v.teamId,gameId:g.id},(out:{notified?:number})=>{toast.success(out?.notified+"명에게 알림을 보냈어요.");setModal(null)})}>미응답자에게 알림 보내기</button></div>}{manager&&g.status==="scheduled"&&<p className="data-note">앱 알림함으로 전달돼요. 같은 경기는 6시간에 한 번 보낼 수 있어요.</p>}{manager&&g.status==="completed"&&<button className="btn btn-green" style={{marginTop:18}} onClick={()=>setModal({kind:"attendance",gameId:g.id})}>{side.attendanceFinal?"출석 수정":"출석 확인 · 확정"}</button>}</TabsContent>
 ...
 {(modal?.kind==="createTeam"||modal?.kind==="editTeam")&&<form className="form-grid" onSubmit={e=>{e.preventDefault();save({...form,type:modal.kind,teamId:v.teamId})}}>{label("팀 이름","name")}<label>활동 지역<Picker value={form.region} onChange={(x:string)=>field("region",x)} options={REGIONS}/></label><div className="two-col fields"><label>주 경기 형식<Picker value={form.format??"11인제"} onChange={(x:string)=>field("format",x)} options={FORMATS}/></label><label>팀 실력<Picker value={levelOf(form.level)} onChange={(x:string)=>field("level",x)} options={LEVELS}/></label></div>{label("주로 뛰는 때","days")}<p className="data-note">예: 일요일 오전, 토요일 저녁. 상대팀을 찾을 때 보여요.</p><label>팀 소개<textarea value={form.description??""} onChange={e=>field("description",e.target.value)}/></label>{submit(modal.kind==="createTeam"?"운영자에게 등록 신청":"팀 정보 저장")}</form>}
 ...
 {modal?.kind==="findTeam"&&<div className="gap-grid">{v.teams.filter((t:Row)=>t.status==="active"&&!v.mine.some((m:Row)=>m.teamId===t.id&&m.status==="active")).map((t:Row)=><div className="row between" key={t.id}><div><strong>{t.name}</strong><p className="data-note">{t.region}</p></div><button className="btn" disabled={v.mine.some((m:Row)=>m.teamId===t.id&&m.status==="pending")} onClick={()=>setModal({kind:"joinTeam",team:t})}>{v.mine.some((m:Row)=>m.teamId===t.id&&m.status==="pending")?"승인 대기":"가입 신청"}</button></div>)}<p className="data-note">가입할 수 있는 승인된 팀을 표시합니다.</p></div>}
 ...
 {modal?.kind==="notifications"&&<>{v.notifications.length?[...v.notifications].reverse().map((n:Row)=><div className="notification-item" key={n.id}><strong className="small">{n.title}</strong><p>{n.body}</p><small>{koreanDate(n.at)} {time(n.at)}</small>{n.gameId&&v.games.some((g:Row)=>g.id===n.gameId)&&<button className="text-link" onClick={()=>setModal({kind:"game",id:n.gameId})}>경기 보기</button>}</div>):<Empty title="새로운 알림이 없어요" description="팀 가입과 경기 변경 소식을 이곳에서 확인할 수 있어요."/>}</>}
