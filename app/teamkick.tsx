...
export default function TeamKick({resetToken="",verifyToken="",kakaoNote=""}:{resetToken?:string;verifyToken?:string;kakaoNote?:string}){
 ...
 useEffect(()=>{
  const go=(name?:string|null)=>{if(name&&VIEWS.includes(name))setView(name)};
 ...
 {view==="home"&&<div className="dashboard"><div className="main-column">{next?<><NextMatch v={v} g={next} open={()=>setModal({kind:"game",id:next.id})}/><Vote v={v} g={next} disabled={busy||team?.status!=="active"} onVote={(value:string)=>run({type:"vote",teamId:v.teamId,gameId:next.id,value})}/></>:<section className="panel"><Empty title="다음 경기를 기다리고 있어요" description={manager?"경기를 등록하고 팀원들의 참여 여부를 모아보세요.":"새 경기가 등록되면 이곳에서 확인할 수 있어요."} action={manager?"첫 경기 만들기":undefined} onClick={()=>setModal({kind:"createGame"})}/></section>}
