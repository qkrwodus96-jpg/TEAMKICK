...
import {MvpPrompt,KingCards} from "./team-play";
...
export function NoticeList({notices,onOpen,dateText=(iso:string)=>localDay(iso).replaceAll("-",".")}:{notices:Row[];onOpen:(n:Row)=>void;dateText?:(iso:string)=>string}){
 ...
 if(!all.length)return <p className="small muted">등록된 공지가 없어요.</p>;
 return <>{shown.map((n:Row)=><div className="notice" key={n.id} tabIndex={0} role="button" onClick={()=>onOpen(n)} onKeyDown={e=>e.key==="Enter"&&onOpen(n)}><p>{n.pinned&&<Pin className="pin"/>}{n.title}</p><span>{dateText(n.at)}</span></div>)}
...
export default function TeamKick({resetToken="",verifyToken="",kakaoNote="",socialSignup=""}:{resetToken?:string;verifyToken?:string;kakaoNote?:string;socialSignup?:string}){
 ...
 {view==="home"&&<div className="dashboard"><div className="main-column"><MvpPrompt v={v} open={(id:string)=>setModal({kind:"game",id,tab:"mvp"})}/>{shown?<><NextMatch v={v} g={shown} open={()=>setModal({kind:"game",id:shown.id})} hasPrev={shownIndex>0} hasNext={shownIndex<reel.length-1} onPrev={()=>setStep(shownIndex-1-baseIndex)} onNext={()=>setStep(shownIndex+1-baseIndex)} position={reel.length>1?shownIndex+1+" / "+reel.length:""}/><Vote v={v} g={shown} disabled={busy||team?.status!=="active"} onVote={(value:string)=>run({type:"vote",teamId:v.teamId,gameId:shown.id,value})}/></>:<section className="panel"><Empty title="다음 경기를 기다리고 있어요" description={manager?"경기를 등록하고 팀원들의 참여 여부를 모아보세요.":"새 경기가 등록되면 이곳에서 확인할 수 있어요."} action={manager?"첫 경기 만들기":undefined} onClick={()=>setModal({kind:"createGame"})}/></section>}
...
function NextMatch({v,g,open,onPrev,onNext,hasPrev,hasNext,position}:any){const team=v.teams.find((t:Row)=>t.id===v.teamId),other=v.teams.find((t:Row)=>t.id===(g.home===v.teamId?g.away:g.home)),side=v.sides.find((s:Row)=>s.gameId===g.id),roster=side?.roster??[],guests=side?.guestRoster??[],
 yes=roster.filter((m:Row)=>currentVote(side,m.id)==="yes").length+guests.length,total=roster.length+guests.length;const dday=seoulDay(Date.parse(g.start))-seoulDay(Date.now());const past=dday<0||g.status==="completed";const score=scoreText(v,g);const intra=isIntra(g);return <section className="next-match"><button type="button" className="match-arrow left" aria-label="이전 경기 보기" disabled={!hasPrev} onClick={onPrev}><ChevronLeft/></button><button type="button" className="match-arrow right" aria-label="다음 경기 보기" disabled={!hasNext} onClick={onNext}><ChevronRight/></button><div className="match-top"><span className="eyebrow">{past?"PAST MATCH":"NEXT MATCH"}</span><span className="row" style={{gap:8}}>{position&&<span className="match-count">{position}</span>}<span className="badge">{past?(g.status==="completed"?"경기 종료":"지난 경기"):dday===0?"TODAY":"D-"+dday}</span></span></div>{intra?<div className="versus"><div className="club"><Crest name={team?.name} logo={team?.logo}/><strong>{team?.name}</strong><span>OUR TEAM</span></div><div className="versus-label">{score||"자체전"}</div><div className="club"><div className="squad-crest" aria-hidden>{SQUAD_NAMES.slice(0,g.squads).map((x:string)=><i key={x}>{x.slice(0,1)}</i>)}</div><strong>{SQUAD_NAMES.slice(0,g.squads).join(" · ")}</strong><span>INTRA-SQUAD</span></div></div>:<div className="versus"><div className="club"><Crest name={team?.name} logo={team?.logo}/><strong>{team?.name}</strong><span>OUR TEAM</span></div><div className="versus-label">{score||"VS"}</div><div className="club"><Crest name={other?.name||g.external} color={other?.color||"orange"} logo={other?.logo}/><strong>{opponent(v,g)||"상대팀 미정"}</strong><span>{g.away?"MATCH CONFIRMED":g.listing==="open"?"LOOKING FOR A TEAM":"FRIENDLY MATCH"}</span></div></div>}<div className="match-meta"><span className="meta-pair"><CalendarDays/>{koreanDate(g.start)}</span><span className="meta-pair"><Clock/>{time(g.start)} – {time(g.end)}</span><span className="meta-pair"><MapPin/>{g.venue}</span></div><div className="match-bottom"><div className="row" style={{gap:8}}><div className="roster-dots">{[...roster,...guests].slice(0,3).map((m:Row)=><i key={m.id}>{m.name.slice(0,1)}</i>)}</div><span><strong style={{color:"#cef18a"}}>{yes}명</strong> 참여 예정 <span style={{color:"#cfe8da"}}> / {total}명{guests.length?" · 용병 "+guests.length:""}</span></span></div><button className="btn btn-lime" onClick={open}>경기 자세히 보기<ArrowUpRight/></button></div></section>}
